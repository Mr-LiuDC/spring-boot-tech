...
public class DepartmentRestTest {
    ...
    public void test001Create() throws Exception {
        params.clear();
        params.add("name", "研发部门");
        String uri = "/deps";
        String result = mockMvc
                .perform(MockMvcRequestBuilders.post(uri).headers(headers).params(params)
                        .contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("研发部门")).andReturn().getResponse()
                .getContentAsString();
        logger.info(result);
    }
    ...
    public void test003UpdateById() throws Exception {
        params.clear();
        params.add("id", "1");
        params.add("name", "财务部门");
        String uri = "/deps";
        String result = mockMvc
                .perform(MockMvcRequestBuilders.put(uri).headers(headers).params(params)
                        .contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("财务部门")).andReturn().getResponse()
                .getContentAsString();
        logger.info(result);
    }
